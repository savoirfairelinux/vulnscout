...
const asVulnerability = (data: any): Vulnerability | [] => {
    ...
    if (typeof data?.epss?.score === "number") vuln.epss.score = Number(data.epss.score)
    if (typeof data?.epss?.percentile === "number") vuln.epss.percentile = Number(data.epss.percentile)
