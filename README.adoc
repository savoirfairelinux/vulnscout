...
The meta-cyclonedx repository is a Yocto meta-layer that enables automatic generation of Software Bill of Materials (SBOMs) in the CycloneDX format during the build process that you can use to easily track and manage vulnerabilities using VulnScout.
The output is generated by collecting metadata about software components and dependencies located in the root filesystem.
