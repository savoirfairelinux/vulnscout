...
If you want to use the non-interactive mode, without the web interface, you can the command with the `--match-condition` argument:
...
[source,shell]
----
./vulnscout --project demo --match-condition "cvss >= 9.0 or (cvss >= 7.0 and epss >= 50%)" --report-template match_condition.adoc
----
