...
# ── R7: importing a multi-target record must keep its whole target set ────

def test_import_of_a_multi_target_record_keeps_a_target_that_already_exists(app):
    """GIVEN target A already exists as its own single-target assessment
    WHEN a version-2 record covering A+B is imported THEN the imported
    assessment still covers both targets.

    De-duplicating target by target would drop A here and store a B-only
    judgement, i.e. an assessment narrower than the one exported. Duplicate
    detection is defined on the complete target set: a subset or superset is
    a different assessment, not a repeat of this one.
    """
    from src.helpers.assessment_io import import_custom_data, build_variant_by_name_map
    from src.models.assessment import Assessment as DBAssessment
    ...
    with app.app_context():
        DBAssessment.create(
            ...
            origin="custom",
            simplified_status="Fixed",
            targets=[(VARIANT_A, _finding_id(PKG_A))],
        )
        payload = {
            "version": 2,
            "assessments": [{
                ...
                "variant_id": None,
                "targets": [
                    {"variant_id": str(VARIANT_A), "variant": "default", "package": PKG_A},
                    {"variant_id": str(VARIANT_B), "variant": "second", "package": PKG_B},
                ],
            }],
        }

        result = import_custom_data(payload, build_variant_by_name_map(PROJECT_UUID))
        ...
        imported = [
            a for a in DBAssessment.get_by_vulnerability(VULN_ID)
            if a.origin == "custom" and len(a.target_rows) > 1
        ]
        assert len(imported) == 1
        assert {(t.variant_id, t.finding_id) for t in imported[0].target_rows} == {
            (VARIANT_A, _finding_id(PKG_A)),
            (VARIANT_B, _finding_id(PKG_B)),
        }


def test_reimporting_the_exact_same_target_set_is_still_skipped(app):
    """The relaxed check must not turn a genuine duplicate into a second copy."""
    from src.helpers.assessment_io import import_custom_data, build_variant_by_name_map
    from src.models.assessment import Assessment as DBAssessment
    ...
    with app.app_context():
        ...
        assert len([
            a for a in DBAssessment.get_by_vulnerability(VULN_ID) if a.origin == "custom"
        ]) == 1


# ── R8: a write must reject a scope naming variants it cannot honour ──────

def test_create_rejects_an_unknown_variant_in_the_scope(client, app):
    """A scope of [valid, unknown] used to succeed silently, storing only the
    valid variant's targets, because only packages observed in *no* variant
    were reported. The write now refuses the request instead."""
    from src.models.assessment import Assessment as DBAssessment

    unknown = str(uuid.uuid4())
    response = client.post(f"/api/vulnerabilities/{VULN_ID}/assessments", json={
        "packages": [PKG_A],
        "status": "fixed",
        "variant_ids": [str(VARIANT_A), unknown],
    })

    assert response.status_code == 400
    assert unknown in response.json["error"]
    with app.app_context():
        assert [a for a in DBAssessment.get_by_vulnerability(VULN_ID) if a.origin == "custom"] == []


def test_create_rejects_variants_from_two_projects(client, app):
    """Variants of different projects can never share an assessment; the
    request is refused up front rather than after silently dropping one."""
    from src.models.project import Project
    from src.models.variant import Variant

    with app.app_context():
        other_project = Project.create("other-project")
        foreign = Variant.create("foreign", other_project.id)
        foreign_id = str(foreign.id)

    response = client.post(f"/api/vulnerabilities/{VULN_ID}/assessments", json={
        "packages": [PKG_A],
        "status": "fixed",
        "variant_ids": [str(VARIANT_A), foreign_id],
    })

    assert response.status_code == 400
    assert "different projects" in response.json["error"]


def test_create_still_accepts_a_valid_multi_variant_scope(client, app):
    """The new scope check must not reject a legitimate sparse selection:
    cairo is observed in variant A only, abc in variant B only."""
    from src.models.assessment import Assessment as DBAssessment
    ...
    with app.app_context():
        ...
        assert {(t.variant_id, t.finding_id) for t in created[0].target_rows} == {
            (VARIANT_A, _finding_id(PKG_A)),
            (VARIANT_B, _finding_id(PKG_B)),
        }
